...
import videoURL from "../assets/videos/sunrise_01.mp4";
...
new p5((sk) => {
  ...
  // Settings
  const avgPos = averageLandmarkPosition(8);
  ...
  function updateAudio(handX) {
    ...
    if (prevHandX !== null && isFinite(prevHandX)) {
      ...
      // Update rate
      const rate = sk.map(absVel, 10, 40, 1, 3); // 5 is the starting point of velocity change
